...
| 제품등록: 이미지                           | 구현     | 파일 업로드 및 URL 입력 모두 지원                      |
...
| 내 정보: 마이페이지/입찰목록/지갑          | 구현     | 내 등록 경매 목록 API 지원 (GET /api/mypage/auctions) |
| 프로필수정: PATCH/탈퇴                     | 구현     | 닉네임 사전중복검사 미지원                             |
| 배송                                       | 미구현   | Placeholder만                                          |
| 관리자: 신고/유저/차단해제/차단경매/통계   | 구현     | GET /api/admin/auctions/blocked, statistics 등         |
...
| 구분           | 항목                         | 비고                          |
| -------------- | ---------------------------- | ----------------------------- |
| 메인           | 위치/근처/조회수             | 백엔드 미지원                 |
| 제품상세       | 1:1 문의                     | 채팅 API 없음                 |
| 제품등록       | 자동연장 토글                | 서버 정책 자동 적용, UI 없음  |
| 경매낙찰 후    | 1:1 채팅·배송 이동           | UI만 있고 API 없음            |
| 배송           | 배송 관리 화면               | API 없음, Placeholder만       |
| 1:1 채팅       | 채팅 페이지                  | API 없음, Placeholder만       |

### 부분 구현 / 제약

| 구분         | 항목                 | 내용                                              |
| ------------ | -------------------- | ------------------------------------------------- |
| 제품상세     | 찜 토글              | 상세 DTO에 `itemId` 없으면 찜 API 호출 안 함     |
| 크레딧충전   | PG 선택 UI           | TOSS 중심, 카카오/네이버/카드 선택은 부분구현    |
| 입출금       | 거래내역 필터        | 거래유형 필터 서버 미지원 → 전체 목록만 표시     |
| 알림         | 삭제·필터·페이징     | 미지원                                           |
| 프로필수정   | 닉네임 중복 검사     | 사전 검사 API 없음, 저장 시 서버 검증만          |

## 알려진 제약사항

- **찜**: 상세 화면 DTO에 `itemId`가 없으면 찜 API를 호출하지 않음(부분구현).
- **이미지**: 경매 등록/프로필 수정에서 파일 업로드 지원.
