...
export function AuctionDetailPage() {
  ...
  const startAt = wsSnapshot.startAt ?? auction?.startAt;
  ...
  const countdownTargetAt = status === "READY" ? startAt : endAt;
  useEffect(() => {
    if (!countdownTargetAt) return;
    const update = () => {
      const sec = Math.max(
        0,
        Math.floor((new Date(countdownTargetAt).getTime() - Date.now()) / 1000)
      );
      setRemaining(sec);
    };
    update();
  ...
  const hourInDay = Math.floor((remaining % 86400) / 3600);
  const minuteInDay = Math.floor((remaining % 3600) / 60);
  const startCountdownStr = `${String(day).padStart(2, "0")}:${String(
    hourInDay,
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
              ...
              <div className="md:text-right">
                {status === "DEADLINE" ? (
                  ...
                  <>
                    <p className="text-xs font-bold text-text-muted uppercase tracking-widest mb-1">
                      {status === "READY" ? "경매 시작까지 남은시간" : "남은 시간"}
                    </p>
