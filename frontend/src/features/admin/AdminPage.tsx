...
export function AdminPage() {
  ...
  const [hotDealTitle, setHotDealTitle] = useState("");
  const [hotDealDescription, setHotDealDescription] = useState("");
  const [hotDealStartPrice, setHotDealStartPrice] = useState("");
  const [hotDealCategoryId, setHotDealCategoryId] = useState<number | null>(null);
  const [hotDealDurationHours, setHotDealDurationHours] = useState("8");
  const [hotDealImageList, setHotDealImageList] = useState<string[]>([]);
  const hotDealFileInputRef = useRef<HTMLInputElement | null>(null);
  ...
  const { data: hotDeals = [], isLoading: hotDealsLoading } = useQuery({
    queryKey: ["admin", "hotDeals"],
    queryFn: () => auctionApi.getHotDeals(),
    enabled: activeTab === "hotDeals",
  });
  ...
  const { data: categories = [], isLoading: categoriesLoading } = useQuery({
    ...
    enabled: activeTab === "categories" || activeTab === "hotDeals",
  });
  ...
  const createHotDeal = useMutation({
    mutationFn: (body: {
      title: string;
      description: string;
      startPrice: number;
      ...
      imageUrls: string[];
    }) => auctionApi.createHotDeal(body),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin", "hotDeals"] });
      queryClient.invalidateQueries({ queryKey: ["auctions", "hotDeals"] });
      addToast("핫딜 경매가 등록되었습니다.", "success");
      setHotDealTitle("");
      setHotDealDescription("");
      setHotDealStartPrice("");
      setHotDealCategoryId(null);
      setHotDealDurationHours("8");
      setHotDealImageList([]);
    },
    onError: (err) => addToast(getApiErrorMessage(err), "error"),
  });

  const cancelHotDeal = useMutation({
    mutationFn: (auctionId: number) => auctionApi.cancelHotDeal(auctionId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin", "hotDeals"] });
      queryClient.invalidateQueries({ queryKey: ["auctions", "hotDeals"] });
      addToast("핫딜 경매가 취소되었습니다.", "success");
    },
    onError: (err) => addToast(getApiErrorMessage(err), "error"),
  });

  const uploadHotDealImages = useMutation({
    mutationFn: (files: File[]) => imageApi.uploadMultiple(files, "item"),
    onSuccess: (urls) => {
      setHotDealImageList((prev) => [...prev, ...urls]);
      ...
      <div className="flex-1 p-6 overflow-auto">
        ...
        {activeTab === "hotDeals" && (
          <div className="space-y-6">
            <h1 className="text-2xl font-bold text-text-main">핫딜 경매 관리</h1>
            <div className="bg-white rounded-xl border border-border p-6">
              <h2 className="text-lg font-semibold text-text-main mb-4">핫딜 경매 등록</h2>
              <form
                className="space-y-4"
                onSubmit={(e) => {
                  e.preventDefault();
                  const title = hotDealTitle.trim();
                  const description = hotDealDescription.trim();
                  const startPrice = Number(hotDealStartPrice.replace(/\D/g, ""));
                  const auctionDuration = Number(hotDealDurationHours.replace(/\D/g, ""));
                  const imageUrls = hotDealImageList;

                  if (!title || !description) {
                    addToast("제목과 설명을 입력해 주세요.", "error");
                    return;
                  }
                  if (!hotDealCategoryId) {
                    addToast("카테고리를 선택해 주세요.", "error");
                    return;
                  }
                  if (!Number.isFinite(startPrice) || startPrice <= 0) {
                    addToast("시작가는 0보다 큰 숫자로 입력해 주세요.", "error");
                    return;
                  }
                  if (!Number.isFinite(auctionDuration) || auctionDuration <= 0) {
                    addToast("경매 시간(시간)을 입력해 주세요.", "error");
                    return;
                  }
                  if (imageUrls.length === 0) {
                    addToast("이미지를 최소 1개 업로드해 주세요.", "error");
                    return;
                  }

                  createHotDeal.mutate({
                    title,
                    description,
                    startPrice,
                    categoryId: hotDealCategoryId,
                    auctionDuration,
                    imageUrls,
                  });
                }}
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div>
                    <label className="block text-sm text-text-muted mb-1">제목</label>
                    <input
                      type="text"
                      value={hotDealTitle}
                      onChange={(e) => setHotDealTitle(e.target.value)}
                      className="w-full rounded-lg border border-border px-3 py-2 text-sm"
                      placeholder="핫딜 경매 제목"
                    />
                  </div>
                  <div>
                    <label className="block text-sm text-text-muted mb-1">카테고리</label>
                    <select
                      value={hotDealCategoryId ?? ""}
                      onChange={(e) =>
                        setHotDealCategoryId(
                  ...
                  <div>
                    <label className="block text-sm text-text-muted mb-1">시작가 (원)</label>
                    <input
                      type="text"
                      value={hotDealStartPrice}
                      onChange={(e) =>
                        setHotDealStartPrice(e.target.value.replace(/\D/g, ""))
                      }
                      className="w-full rounded-lg border border-border px-3 py-2 text-sm"
                      placeholder="10000"
                    />
                  ...
                  <div>
                    <label className="block text-sm text-text-muted mb-1">경매 시간 (시간)</label>
                    <input
                      type="text"
                      value={hotDealDurationHours}
                      onChange={(e) =>
                        setHotDealDurationHours(e.target.value.replace(/\D/g, ""))
                      }
                      className="w-full rounded-lg border border-border px-3 py-2 text-sm"
                      placeholder="8"
                    />
                ...
                <div>
                  <label className="block text-sm text-text-muted mb-1">설명</label>
                  <textarea
                    value={hotDealDescription}
                    onChange={(e) => setHotDealDescription(e.target.value)}
                    className="w-full rounded-lg border border-border px-3 py-2 text-sm min-h-[80px]"
                    placeholder="핫딜 경매 설명"
                  />
                </div>
                <div>
                  <label className="block text-sm text-text-muted mb-2">이미지</label>
                  <input
                    ref={hotDealFileInputRef}
                    type="file"
                    accept="image/*"
                    multiple
                    className="hidden"
                    onChange={(e) => {
                      ...
                      uploadHotDealImages.mutate(validFiles);
                      e.target.value = "";
                    }}
                  />
                  <div className="flex items-center gap-2 mb-3">
                    <Button
                      ...
                      variant="secondary"
                      size="sm"
                      onClick={() => hotDealFileInputRef.current?.click()}
                      loading={uploadHotDealImages.isPending}
                    >
                      파일 업로드
                    </Button>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    {hotDealImageList.map((url, index) => (
                      <div
                        key={`${url}-${index}`}
                        className="relative aspect-square rounded-lg border border-border overflow-hidden group bg-gray-50"
                      >
                    ...
                    <button
                      type="button"
                      onClick={() => hotDealFileInputRef.current?.click()}
                      className="aspect-square rounded-lg border-2 border-dashed border-border flex flex-col items-center justify-center text-text-muted hover:border-primary transition-colors bg-gray-50"
                    >
                      <span className="material-symbols-outlined text-2xl">
                        upload_file
                      </span>
                      <span className="text-[10px] font-semibold">이미지 추가</span>
                    </button>
                  </div>
                  {hotDealImageList.length > 0 && (
                    <p className="mt-2 text-[11px] text-text-muted">
                      첫 번째 이미지가 홈 배너에 대표 썸네일로 노출됩니다.
                    </p>
                  )}
                </div>
                <div className="flex justify-end">
                  <Button type="submit" loading={createHotDeal.isPending}>
                    핫딜 등록
                  </Button>
                </div>
              </form>
            ...
            <div className="bg-white rounded-xl border border-border overflow-hidden">
              ...
              {hotDealsLoading ? (
                ...
                <p className="p-8 text-center text-text-muted">등록된 핫딜 경매가 없습니다.</p>
              ) : (
                <ul className="divide-y divide-border">
                  {hotDeals.map((deal) => (
                    <li
                    ...
                    >
                      <div>
                        <p className="font-semibold">
                          #{deal.auctionId} {deal.title}
                        </p>
                        <p className="text-sm text-text-muted">
                          상태: {deal.status} · 현재가 {formatKrw(deal.currentPrice)}
                        </p>
                        <p className="text-xs text-text-muted mt-1">
                          시작 {formatDateTime(deal.startAt)} · 종료 {formatDateTime(deal.endAt)}
                        </p>
                      </div>
                      <Button
                        variant="danger"
                        size="sm"
                        disabled={deal.status !== "READY"}
                        loading={cancelHotDeal.isPending}
                        onClick={() => cancelHotDeal.mutate(deal.auctionId)}
                      >
                        취소
                      </Button>
                    </li>
